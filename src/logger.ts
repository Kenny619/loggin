...
class Log {
	...
	static async info(eventName: string, message: string) {
		try {
			await Log.record("INFO", eventName, message);
		} catch (e) {
	...
	static async warn(eventName: string, message: string) {
		try {
			await Log.record("WARN", eventName, message);
		} catch (e) {
	...
	static async error(eventName: string, message: string) {
		try {
			await Log.record("ERROR", eventName, message);
		} catch (e) {
			throw new Error(`Log error failed ${e}`);
		}
	}

	static async fatal(eventName: string, message: string) {
		try {
			await Log.record("FATAL", eventName, message);
		} catch (e) {
			throw new Error(`Log fatal failed ${e}`);
		}
	}
}
...
try {
	...
	throw new Error(`Log init failed.  ${e}`);
}
